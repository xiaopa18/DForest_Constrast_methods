...
public:
    ...
    KNN_IO();
    KNN_IO(Point &, mainRef_Point &, priority_queue<pair<double, unsigned int> > &,CalCirclePos &, vector<CalCirclePos> &, unsigned, int8_t (*)[20000], unsigned, int &, double &);
    ...
    void DoRangeQueryForKNN(unsigned, vector<vector<int> > &, unsigned long long, mainRef_Point &, priority_queue<pair<double, unsigned int> > &, int8_t (*)[20000], int &,double &);
};

KNN_IO::KNN_IO(){}

KNN_IO::KNN_IO(Point &KNN_pt, mainRef_Point &mainRef_point, priority_queue<pair<double, unsigned int> > &KNNRes_queue, CalCirclePos &mainRefPtCircle, vector<CalCirclePos> &ref_query, unsigned k, int8_t (*arr)[20000], unsigned clu_id, int &page,double &io_time)
{
    ...
    DoRangeQueryForKNN(i, rank_list, queryRange, mainRef_point, KNNRes_queue, arr, page, io_time);
}

void KNN_IO::DoRangeQueryForKNN(unsigned i, vector<vector<int> > &rank_list, unsigned long long queryRange, mainRef_Point &mainRef_point, priority_queue<pair<double, unsigned int> > &KNNRes_queue, int8_t (*arr)[20000], int &page, double &io_time){
